...
if iscell(VarName)
    for m = 1:length(VarName)
        ...
        VarStruct.(VarName{m}) = recursive_extract_Var(folderPath, regexPattern,VarName{m});
        % Check if the Var variable has been extracted
        if ~isempty(VarStruct.(VarName{m}))
            disp(['Variable ', VarName{m}, ' has been extracted from all qualifying files!',newline]);
        else
            disp(['No files with the correct naming format were found or the files do not contain the ', VarName{m}, ' variable!',newline]);
        end
    end
else
    % Call the recursive function to traverse folders and subfolders
    VarStruct.(VarName) = recursive_extract_Var(folderPath, regexPattern,VarName);
    % Check if the Var variable has been extracted
    if ~isempty(VarStruct.(VarName))
        disp(['Variable ', VarName, ' has been extracted from all qualifying files!',newline]);
    else
        disp(['No files with the correct naming format were found or the files do not contain the ', VarName, ' variable!',newline]);
    end
end
